...
class ESOInterface:
    ...
    Usage:
        interface = ESOInterface(eso)
        
        # In control loop:
        state = interface.get_state()
        disturbances = interface.get_disturbances_as_control()
        
        # Controller uses:
        u_nominal = controller.compute(state.position, state.velocity, ...)
        u_compensated = u_nominal + disturbances
    """
    
    def __init__(self, eso, mass: float, inertia: Optional[np.ndarray] = None):
        """
        Args:
            eso: ESO object (9-state or 12-state)
            mass: drone mass (kg)
    ...
    def _detect_eso_type(self) -> str:
        """Detect whether ESO is 9-state or 12-state."""
    ...
    def get_disturbances_as_control(self, roll: float, pitch: float, yaw: float) -> ESODisturbanceControl:
        ...
        # Extract body-frame disturbances
        # In body frame: x=forward, y=right, z=down (NED) or z=up (NWU)
        # For Crazyflie with z-up convention:
        thrust_disturbance = d_f_body[2]  # Disturbance in thrust direction
        
        # Lateral forces create torques via moment arms
        # For small drones, this is typically negligible
        # If you have torque disturbances estimated (18-state ESO):
        if self.eso_type == "18-state":
            torque_disturbance = self.eso.z[15:18].copy()
        else:
            # Estimate torque from lateral forces (simplified model)
            # τ ≈ r × F where r is moment arm
            # Crazyflie arm length (meters)
            arm_length = 0.0465

            # Body-frame force disturbance:
            #   d_f_body = R_world_to_body @ d_f_world
